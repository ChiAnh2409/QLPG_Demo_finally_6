...
(
    ...
    DaDiemDanh bit,
    foreign key(id_CTDKGoiTap) references dbo.ChiTietDK_GoiTap(id_CTDKGoiTap)
)
go
...
WHERE ct.NgayKetThuc IS NOT NULL
	  ...
	  DECLARE @selectedYear INT; -- Declare a variable to hold the selected year value
SET @selectedYear = NULL; -- Set the default value, assuming NULL if not provided

-- Common Table Expression (CTE) to filter ChiTietDK_GoiTap based on the selected year
WITH chitietdoanhthu AS (
    SELECT *
    FROM ChiTietDK_GoiTap
    WHERE (@selectedYear IS NULL OR (NgayBatDau IS NOT NULL AND YEAR(NgayBatDau) = @selectedYear))
)

-- Main query to group by month and calculate the rounded sum of ThanhTien for each month
SELECT
    MONTH(NgayBatDau) AS Month,
    ROUND(SUM(ISNULL(ThanhTien, 0)), 3) AS tổng_doanh_thu
FROM
    chitietdoanhthu
GROUP BY
    MONTH(NgayBatDau)
